...
def fetch_multi_timeframe_data():
    ...
    # 타임프레임별 데이터 수집 설정
    timeframes = {
        "15m": {"timeframe": "15m", "limit": 100},  # 지표 계산을 위해 약간 더 많은 데이터 수집
        "1h": {"timeframe": "1h", "limit": 100},
        "4h": {"timeframe": "4h", "limit": 100}
    }
...
def fetch_ethereum_news():
    """
    이더리움 관련 최신 뉴스를 가져옵니다
    
    SERP API를 사용해 Google 뉴스에서 이더리움 관련 최신 뉴스 10개를 가져옵니다.
...
while True:
    try:
        ...
        # ===== 3. 포지션이 없는 경우 처리 =====
            ...
            # 최신 이더리움 뉴스 수집
            recent_news = fetch_ethereum_news()
            ...
            # ===== 6. AI 트레이딩 결정 요청 =====
            # AI 분석을 위한 시스템 프롬프트 설정 (이더리움으로 수정)
            system_prompt = """
You are a crypto trading expert specializing in multi-timeframe analysis and news sentiment analysis applying Kelly criterion to determine optimal position sizing, leverage, and risk management for ETHEREUM (ETH) trading.
You adhere strictly to Warren Buffett's investment principles:

**Rule No.1: Never lose money.**
**Rule No.2: Never forget rule No.1.**

Analyze the market data across different timeframes (15m, 1h, 4h), recent news headlines, and historical trading performance to provide your trading decision for ETHEREUM.
...
2. Assess the current market condition across all timeframes for ETHEREUM:
   ...
   - News sentiment: Analyze recent Ethereum news article titles for bullish or bearish sentiment
   - **RSI (RSI_14)**: Identify overbought (>70) or oversold (<30) conditions. Look for bullish/bearish divergences.
   - **MACD (MACD_12_26_9, MACDh_12_26_9, MACDs_12_26_9)**: Analyze the MACD line, signal line, and histogram. A crossover of the MACD line above the signal is bullish; below is bearish. The histogram indicates momentum.
   - **Bollinger Bands (BBU_20_2.0, BBM_20_2.0, BBL_20_2.0)**: Observe the price relative to the bands. Price touching the upper band can indicate overbought conditions, while touching the lower band can mean oversold. A "squeeze" (narrowing bands) suggests low volatility and a potential breakout.

3. Based on your analysis, determine:
   - Direction: Whether to go LONG or SHORT for ETHEREUM
...
5. Determine optimal leverage for ETHEREUM:
...
6. Set optimal Stop Loss (SL) and Take Profit (TP) levels for ETHEREUM:
...
8. Provide reasoning:
   - Explain the rationale behind your trading direction, leverage, and SL/TP recommendations for ETHEREUM
...
"""
            ...
            response = client.chat.completions.create(
                model="gpt-5",  # GPT-5 모델 사용 (실제로는 gpt-4-turbo 또는 최신 버전 사용)
                messages=[
            ...
            try:
                ...
                # 최소 주문 금액 확인 (최소 20 USDT - 이더리움은 BTC보다 낮음)
                if investment_amount < 20:
                    investment_amount = 20
                    print(f"최소 주문 금액(20 USDT)으로 조정됨")
