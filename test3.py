...
# ===== 더 이상 사용하지 않는 함수 (AI가 직접 판단) =====
# def calculate_data_quality_score(): 
#     이 함수는 제거됨 - AI가 data_status를 보고 직접 판단하도록 변경

def enhanced_trading_analysis():
    """기존 메인 루프에서 호출할 강화된 분석 함수 (간소화)"""
    
    # 종합 시장 분석 수행 (데이터 상태 포함)
...
1. 📈 MOMENTUM INDICATORS (40%) - Primary technical signals
2. 💭 MARKET SENTIMENT (25%) - Investor psychology & positioning  
3. 🔗 ON-CHAIN DATA (20%) - Network activity & whale movements
4. 📊 MACRO CORRELATION (10%) - Traditional market context
5. 📰 NEWS SENTIMENT (5%) - Short-term catalysts

COMPREHENSIVE DECISION PROCESS:
1. TECHNICAL FOUNDATION: Check momentum indicators across timeframes
   - RSI levels for oversold/overbought conditions
   - MACD for trend changes and momentum
   - Stochastic for precise entry timing
   - Williams %R for reversal confirmation
   - Multi-timeframe alignment for higher confidence

2. SENTIMENT VALIDATION: Analyze market psychology
   - Funding rates for leverage bias (contrarian opportunities)
   - Long/Short ratios for crowd positioning
   - Open Interest trends for market commitment
   - Liquidation pressures for potential squeezes

3. ON-CHAIN CONFIRMATION: Evaluate network fundamentals
   - Exchange flows for accumulation/distribution
   - Whale activity for potential catalysts  
   - Gas prices for network demand and volatility
   - Transaction patterns for ecosystem health

4. MACRO CONTEXT: Consider broader market environment
   - NASDAQ correlation for risk appetite
   - DXY strength for dollar impact
   - Economic indicators for market sentiment

5. NEWS CATALYST: Factor in immediate market drivers
   - Breaking news impact on sentiment
...
Strong Buy Setup:
- Technical: RSI 25 + MACD bullish cross + Stoch turning up (40% weight)
- Sentiment: Funding 0.02% + L/S ratio 2.1 (contrarian setup) (25% weight)  
- On-chain: Exchange outflows + low gas (accumulation) (20% weight)
- Macro: NASDAQ +1.2% + DXY -0.5% (risk-on) (10% weight)
- News: Positive ETF news (5% weight)
→ HIGH CONVICTION LONG 0.4-0.5

Mixed Signals:
- Technical: RSI 45 + MACD flat (neutral momentum)
- Sentiment: Funding neutral + L/S balanced  
- On-chain: Strong whale buying
- Macro: Mixed traditional markets
→ MODERATE LONG 0.2-0.25 (on-chain strength compensates)

Conflicting Data:
- Technical: Strong bullish setup
- Sentiment: Extreme long bias (0.03% funding)
- On-chain: Major exchange inflows
→ REDUCED LONG 0.1-0.2 (technical vs fundamental conflict)

DATA RELIABILITY IMPACT:
Adjust confidence based on data_status:
- All sources reliable (HIGH): Use full analysis framework
- Mixed reliability: Weight reliable sources higher
- Low reliability: Focus on most trustworthy data, reduce position sizes

ADAPTIVE POSITION SIZING:
Base Position (from technical + sentiment): 0.1 - 0.4
Adjustments:
- Strong on-chain confirmation: +0.1
- Macro alignment: +0.05
- News catalyst: +0.05
- Data reliability issues: -0.1 to -0.2
- Maximum position: 0.6 regardless of signals

RISK MANAGEMENT:
- High conviction (all signals align): Standard stops 15-25%
- Medium conviction (some conflict): Tighter stops 12-20%
- Low data reliability: Conservative stops 10-18%
- High volatility environment: Wider stops +3-5%

DECISION PRIORITIES:
1. Never ignore strong technical signals completely
2. Strong sentiment can validate weak technical setups
3. On-chain data provides valuable medium-term context
4. Macro helps with overall market timing
5. News creates short-term opportunities but shouldn't drive strategy

RESPONSE FORMAT (JSON ONLY):
{
  "direction": "LONG",
  "recommended_position_size": 0.3,
  "recommended_leverage": 18,
  "stop_loss_percentage": 0.16,
  "take_profit_percentage": 0.32,
  "reasoning": "Balanced analysis mentioning key factors from multiple categories that influenced the decision"
}

Remember: Balance all factors - no single indicator should dominate unless extremely compelling.
"""
...
print("✅ Smart AI Decision: Self-evaluating data reliability")
print("✅ Adaptive Risk Management: AI adjusts based on data quality")
    ...
    try:
        ...
        # ===== 5. 포지션이 없는 경우 처리 =====
            ...
            market_analysis = {
                ...
                # 🆕 데이터 상태 리포트 (AI가 직접 판단하도록)
                "data_status": comprehensive_analysis["data_status"],
            ...
            try:
                ...
                if comprehensive_events.get("comprehensive_event"):
                    ...
                    print(f"📊 Data Sources: {comprehensive_analysis['data_status']['summary']}")
                    print(f"🔄 Overall Reliability: {comprehensive_analysis['data_status']['overall_reliability']}")
                else:
                    print(f"📊 Data Sources: {comprehensive_analysis['data_status']['summary']}")
                    print(f"🔄 Overall Reliability: {comprehensive_analysis['data_status']['overall_reliability']}")
                ...
                if action == "long":
                    ...
                    print(f"📊 Data Reliability: {comprehensive_analysis['data_status']['overall_reliability']}")
                    print(f"🔄 Sources: {comprehensive_analysis['data_status']['summary']}")
...
print("📊 Features: Technical + Etherscan + Macro + News + Smart-AI")
...
print("🧠 Smart AI: Self-evaluating data reliability and adaptive sizing")
